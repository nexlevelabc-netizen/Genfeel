        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {projects.map((project, i) => (
            <ScrollReveal key={project.name} delay={i * 0.15} scale={0.95}>
              <a href={project.link} target="_blank" rel="noopener noreferrer" className="block relative rounded-card overflow-hidden group aspect-[4/3]">
                ...
                <div className="absolute inset-0 bg-ink/0 group-hover:bg-ink/85 transition-all duration-400 flex items-center justify-center">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-400 text-center">
                    <p className="font-body text-sm font-medium mb-2" style={{ color: '#ec4899' }}>{project.category}</p>
                    <h3 className="font-display text-[28px] text-white mb-4">{project.name}</h3>
                    <span className="inline-flex items-center gap-2 font-body text-sm font-medium" style={{ color: accentColor }}>
                      View Project
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 max-w-4xl mx-auto">
          {metrics.map((m) => (
            <div key={m.label} className="text-center">
              ...
              <p className="font-body text-sm text-white-muted mt-2">{m.label}</p>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <section className="relative z-10 bg-cream-dark py-section">
      <div className="container-content text-center">
        <ScrollReveal>
          <h2 className="font-display text-4xl lg:text-6xl font-medium text-ink max-w-2xl mx-auto leading-tight">
            Have a project in mind?
          </h2>
          <p className="font-body text-lg text-ink-light mt-6 max-w-lg mx-auto">
